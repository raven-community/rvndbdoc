...
**A Turing Complete Query Language for Building Immutable APIs on Ravencoin**
...
Bitquery is a query language implementation for [RvnDB](http://rvndb.ravencoin.online), a universal ravencoin database.

Once the RvnDB [crawler](http://docs.rvndb.ravencoin.online/docs/crawler) and [indexer](indexer) are up and running, you can query your RvnDB node any way you want. It's just a MongoDB instance after all.

Because the **WRITE** (crawling + indexing) is deterministic (like a mathematical function), we can be sure that querying RvnDB will be equivalent to querying Ravencoin.

However this is not enough. The main goals of RvnDB are:

1. **Transparency:** RvnDB should help applications build transparent and instantly open sourced applications
2. **Portability:** Applications powered by RvnDB must be portable. If one host dies, the "spirit" (the application logic) should be able to seamlessly migrate over to another host.
3. **Interoperability:** Applications powered by RvnDB should be able to talk to one another through open API mashups.
...
This is why RvnDB has its own query language (and query engine).
...
## 3. RvnDB Document Format

Before looking at the query language, first make sure you understand how a ravencoin transaction is decoded, transformed and stored into a query-optimized format in RvnDB.

You can learn about RvnDB document format in the [Indexer page](indexer)
...
Because every application can have their own custom encoding scheme per script push data, every RvnDB query needs to contain the encoding info in addition to the query logic.
...
Then check out the [RvnDB Introduction](intro_v3#query-examples) to see some examples.

If you want to learn more about how these work behind the scenes, read on.

#### ii. Under the Hood

Because RvnDB stores everything as base64 (b-attributes) and UTF8 (s-attributes) encoding, if we want to query with a hex encoded value, we need to manually specify that it's hex-encoded, so rvnquery knows to transform it to base64 before making the query. 
...
In version 3, RvnDB introduces a new concept called **"virtual attribute"** to make the query much simpler, flexible, and extensible. Here's how it works:
...
1. RvnDB introduces virtual variables with **h-prefix** (h0, h1, h2, h3, ...) which are hex encoded representations of the b0, b1, b2, b3, etc. counterparts.
...
To learn how to query RvnDB, you can just think of this as a regular MongoDB instance.
...
There are currently two separate collections in RvnDB: `c` (confirmed) and `u` (unconfirmed).
...
```
{
  ...
  "c": [{
    "token_symbol": "TEST",
    "token_name": "TEST",
    "document_url": "ravencoinfiles:b86b4bcbab7cd787b1c893ca101250c8c467dbba4df229b118218bd8a9e85a92"
  }, {
    "token_symbol": "VOTE",
    "token_name": "An Election",
    "document_url": "ravencoinfiles:a90e59ef7ca66b25b6ba98d028198ae222a8229804c4b0b3bc0b1bafe104738a"
  }, {
    "token_symbol": "WuCash",
...
Now that it's possible to include the processing logic directly inside the query itself, you can make RvnDB return a response in EXACTLY the format you want.

This means now you can use RvnDB to **build your own API**.
...
1. **DB Query:** A [query into ravencoin's state](#b-query) which always returns **deterministic results**.
...
This means we have just created a new piece of immutable data from ravencoin, through [alchemy](#c-processing)!
